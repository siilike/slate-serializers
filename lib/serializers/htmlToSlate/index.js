...
const deserialize = ({ el, config = default_1.config, index = 0, childrenLength = 0, context = '', }) => {
    ...
    if (nodeName === 'br' && config.convertBrToLineBreak && context !== 'preserve') {
        const isWithinTextNodes = ((_a = parent.prev) === null || _a === void 0 ? void 0 : _a.type) === htmlparser2_1.ElementType.Text && ((_b = parent.next) === null || _b === void 0 ? void 0 : _b.type) === htmlparser2_1.ElementType.Text;
        return [(0, slate_hyperscript_1.jsx)('text', { text: isWithinTextNodes ? '' : '\n' }, [])];
    }
    ...
    if (config.elementTags[nodeName]) {
        ...
        if (style) {
            Object.keys(style).forEach((slateKey) => {
                ...
                const cssValue = (0, domhandler_2.extractCssFromStyle)(parent, cssProperty);
                if (cssValue) {
    ...
    if (config.textTags[nodeName] || el.type === htmlparser2_1.ElementType.Text) {
        ...
        if ((config.filterWhitespaceNodes && (0, whitespace_1.isAllWhitespace)(text) && !childrenContext) || text === '') {
            return null;
        }
...
const gatherTextMarkAttributes = ({ el, config = default_1.config }) => {
    ...
    if (children.length > 0) {
        ...
        if (children.length === 1 && (0, domutils_1.getChildren)(children[0]).length > 0) {
            allAttrs = {
                ...allAttrs,
